...
def discriminator_loss(logits_real, logits_fake):
    ...
    ones_target = torch.ones_like(logits_real).type(dtype)
    zeros_target = torch.zeros_like(logits_fake).type(dtype)
    loss = bce_loss(logits_real, ones_target)
    loss += bce_loss(logits_fake, zeros_target)
...
def generator_loss(logits_fake):
    ...
    fake_target = torch.ones_like(logits_fake).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = (scores_real - 1)**2 / 2 + scores_fake**2 / 2
    loss = loss.mean()
