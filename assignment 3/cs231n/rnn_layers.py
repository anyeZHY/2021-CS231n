...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    out = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
    next_h = np.tanh(out)
...
def rnn_step_backward(dnext_h, cache):
    ...
    exp = np.exp(2 * out)
    dnext_h = dnext_h * 4 / (2 + exp + 1/exp)
    dx = dnext_h.dot(Wx.T)
    dWx = x.reshape(x.shape[0], -1).T.dot(dnext_h)
    dprev_h = dnext_h.dot(Wh.T).reshape(prev_h.shape)
    dWh = prev_h.reshape(prev_h.shape[0], -1).T.dot(dnext_h)
...
def rnn_backward(dh, cache):
    ...
    N, T, D = x.shape
    _, H = h0.shape
    dx, dh0 = np.zeros_like(x), np.zeros_like(h0)
    dWx, dWh, db = np.zeros_like(Wx), np.zeros_like(Wh), np.zeros_like(b)
    dprev_h = 0
    for index in range(T):
        i = T - index - 1
        prev_h = h[:,i-1,:] if i!=0 else h0
        out = np.dot(x[:,i,:], Wx) + np.dot(prev_h, Wh) + b
        cache = (x[:,i,:], prev_h, Wx, Wh, b, out)
        
        dxi, dprev_h, dWxi, dWhi, dbi = rnn_step_backward(dh[:,i,:]+dprev_h, cache)
        dWx += dWxi
        dWh += dWhi
        db += dbi
        dx[:,i,:] = dxi
