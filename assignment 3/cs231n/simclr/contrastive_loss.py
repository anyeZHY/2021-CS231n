...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2*N):
            if j!=k:
                deno_k += torch.exp(sim(z_k, out[j])/tau)
            if j!=k+N:
                deno_kN += torch.exp(sim(z_k_N, out[j])/tau)
        total_loss += -torch.log(torch.exp(sim(z_k, z_k_N)/tau) / deno_k)
        total_loss += -torch.log(torch.exp(sim(z_k_N, z_k)/tau) / deno_kN)
