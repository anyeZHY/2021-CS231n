...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        v_j = torch.float_power(10000, even / embed_dim)
        i = torch.arange(max_len)
        pe[0,:,::2] = torch.sin(torch.outer(i, v_j))
        pe[0,:,1::2] = torch.cos(torch.outer(i, v_j))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        XQ = self.query(query).reshape(N, S, H, D//H)
        XK = self.key(key).reshape(N, T, H, D//H)
        XV = self.value(value).reshape(N, T, H, D//H)
        XQ = XQ.permute([0, 2, 1, 3]) # of shape (N, H, T, D/H)
        XK = XK.permute([0, 2, 3, 1]) # of shape (N, H, D/H, T)
        XV = XV.permute([0, 2, 1, 3]) # of shape (N, H, T, D/H)
        
        XQXK = torch.matmul(XQ, XK) / math.sqrt(D/H) # of shape (N, H, T, T)
        if attn_mask is not None:
          XQXK = XQXK.masked_fill(attn_mask == 0, -1e9)
        softmax = F.softmax(XQXK, dim=3)
        softmax = self.dropout(softmax)
        Y = torch.matmul(softmax, XV) # of shape (N, H, T, D/H)
